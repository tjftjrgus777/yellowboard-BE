...
public class AuthServiceImplement implements AuthService{
    ...
    public ResponseEntity<? super SignInResponseDto> signIn(SignInRequestDto dto) {
        ...
        try {
            ...
            if (user == null) throw new NotFoundException(NOT_EXISTED_USER);

            boolean checkPasswordIsCorrect = passwordEncoder.matches(requestedPassword, user.getPassword());
            if (!checkPasswordIsCorrect) return SignInResponseDto.signInFail();
     ...
     * 무효화된 토큰을 처리하는 메서드.
     * <p>
     * 1. 사용자 이메일과 연관된 데이터(토큰)를 Redis에서 삭제합니다.<p>
     * 2. 무효화된 액세스 토큰을 블랙리스트에 추가하고, 지정된 시간 동안 유지합니다.<p>
     * <p>
     * - 토큰 삭제: 사용자가 로그아웃하거나 토큰이 무효화될 때, 즉시 해당 토큰을 삭제하여 더 이상 유효하지 않게 합니다.
     * - 블랙리스트 추가: 이미 발행된 토큰이 유효기간 내에 재사용되는 것을 방지합니다. 이를 통해 네트워크 지연이나 기타 이유로 클라이언트에 전달된 토큰이 다시 사용되지 않도록 보장합니다.
     * <p>
